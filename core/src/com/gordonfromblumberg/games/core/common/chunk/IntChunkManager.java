...
public class IntChunkManager<T> {
    private final IntChunk<T>[][] chunks;
    ...
    public IntChunkManager(int gridWidth, int gridHeight, int chunkSize) {
        ...
        this.chunks = new IntChunk[chunkGridWidth][chunkGridHeight];
        for (int i = 0; i < chunkGridWidth; ++i) {
            for (int j = 0; j < chunkGridHeight; ++j) {
                this.chunks[i][j] = new IntChunk<>(i, j, chunkSize, gridWidth, gridHeight);
            }
        }
    }
    ...
    public Array<T> findObjectsUnderLine(int x1, int y1, int x2, int y2) {
        ...
        final IntChunk<T>[][] chunks = this.chunks;
        int dx = x2 - x1;
        if (dx < 0)
            dx += gridWidth;
